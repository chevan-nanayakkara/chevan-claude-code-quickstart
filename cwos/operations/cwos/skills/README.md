...
- **[`spoke-cold-start/`](spoke-cold-start/SKILL.md)** — The narrow counterpart to `cwos-cold-start`, for a session running in a spoke or development repo rather than the hub. Loads only what such a session needs: the hub's `AICONFIG.md` behavioral charter, the feedback memories that govern work anywhere, the handoff protocol plus this surface's waiting inbox, and the repo's own documentation. Deliberately refuses to load the hub's work state, because a spoke session that absorbs hub structure starts reproducing it inside a code repository. States the audience boundary back (what belongs in the repo vs. the hub) and declares the hub write scope as handoffs-only. Read-only; target output 150-250 words.
...
- **[`refresh-work-management/`](refresh-work-management/SKILL.md)** — Three-step orchestrator that refreshes both `0-work-status.md` (via the repo's `work-status-dashboard` skill) and `0-project.md` (via `open-projects`), then produces a priority outline (via `prioritize-open-projects`). **This is the recommended common-case invocation** for the full work-management workflow; the three underlying skills remain available for edge cases. Authored 2026-06-02 (v1.2.0) as a two-step orchestrator; extended to three steps including prioritization in v1.6.0 (2026-06-16). Pass `--skip-prioritize` for the prior two-step behavior. Requires the repo to author its own `work-status-dashboard` skill (table set is repo-specific).
...
- **[`handoff-write/`](handoff-write/SKILL.md)** — Author a cross-surface handoff as a file rather than a chat message: gathers `from` / `to` / `project` / `subject` / `kind` / `response-expected`, computes the `YYYY-MM-DD-HHMM-<from>-to-<to>.md` filename, applies the routing rule, and writes a structured body (Context / The ask / Standing rules / Return handoff / Origin). Enforces the rule that handoffs never land in a spoke or development repo.
- **[`handoff-inbox/`](handoff-inbox/SKILL.md)** — Read-only companion. Lists handoffs addressed to the current session's surface and not yet answered, auto-detecting the surface from the working directory. Frontmatter-first parse with a filename fallback; reply-tracking via `in-reply-to` where present, heuristic and labeled as such where not. Scans the hub, not the local repo.

**Document production:**

- **[`document-export/`](document-export/SKILL.md)** — Convert a self-contained HTML design-document bundle (`*.dc.html` + `_ds/` design system + `assets/` + `uploads/`) into an email-ready PDF and a Google-Docs-friendly `.docx`. Codifies two render paths (headless Chrome + Ghostscript for PDF; pandoc + a brand reference.docx for Word), seven silent-failure modes with cause and fix (invisible table borders after Docs import, stray bookmark markers, scrambled contents, half-width images, run-together numbers, missing fonts, font substitution), and a structural + visual + content-parity verification checklist. Ships a bundled `scripts/` toolkit (~900 lines Python/Lua) sanctioned by `agent-skills-standard.md`. Brand configuration is a JSON file, not hard-coded — `scripts/brand.example.json` is a neutral placeholder; copy to `brand.<project>.json` and derive values from the bundle's own `_ds/*/tokens/colors.css`. First `scripts/`-bundled skill in the starter (ported from chevan-content v1.2.0 skills README, August 7, 2026).
